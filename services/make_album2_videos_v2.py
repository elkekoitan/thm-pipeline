...
BASE2 = "/home/ubuntu/muzik/album2"
BASEV2 = "/home/ubuntu/muzik/album2_v2"
W, H = 1920, 1080
FPS = 24
...
def build_song(idx, key, title, scenes, audio_dur):
    mp4 = os.path.join(BASEV2, f"{idx}_{key}_video.mp4")
    short = os.path.join(BASEV2, f"{idx}_{key}_short.mp4")
    ...
    for i, s in enumerate(scenes):
        im = load_scene(os.path.join(BASE2, s))
        ...
        nxt_im = load_scene(os.path.join(BASE2, scenes[i + 1])) if i < n_scenes - 1 else None
        for fi in range(n):
        ...
        # re-mux to a clean container (xfade fails to open raw jpg-origin muxes)
        # and downscale to 1280x720 to keep memory well within sandbox limits
        remux = clip.replace(".mp4", "_r.mp4")
        subprocess.run(
            ["ffmpeg", "-y", "-threads", "1", "-i", clip,
             "-vf", "scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:(ow-iw)/2:(oh-ih)/2", "-c:v", "libx264", "-preset", "ultrafast", "-crf", "21", remux],
            check=True, capture_output=True,
        )
        os.replace(remux, clip)
        ...
        for f in glob.glob(os.path.join(tmp, "f*.jpg")):
            os.remove(f)
        clips.append(clip)
    audio = os.path.join(BASEV2, f"{idx}_{key}.mp3")
    mp4 = os.path.join(BASEV2, f"{idx}_{key}_video.mp4")
    if len(clips) == 1:
        import time as _t
        ok = False
        for _attempt in range(3):
            r = subprocess.run([
                "ffmpeg", "-y", "-threads", "1", "-i", clips[0], "-i", audio, "-t", str(audio_dur),
                "-vf", "format=yuv420p", "-c:v", "libx264", "-preset", "ultrafast", "-crf", "21",
            ...
            if r.returncode == 0:
                ok = True
                break
            print(f"[warn] clip encode attempt {_attempt+1} failed, retrying"); _t.sleep(15)
        if not ok:
            raise RuntimeError(f"clip encode failed for {key}")
    else:
        # xfade chain of clips; use comma-chain after last xfade (no intermediate
        # [v{i}] labels — proven to be the only pattern ffmpeg accepts here)
        per_d = per / FPS
        norm_inputs = []
        norm_fc = []
        ...
        for i in range(1, len(clips)):
            ...
            xfade_parts.append(
                f"[n{i-1}][n{i}]xfade=transition=fade:duration={FADE_FRAMES / FPS:.3f}:offset={off:.3f}"
            )
        ...
        for _attempt in range(3):
            r = subprocess.run(
                [
                    "ffmpeg", "-y", *norm_inputs, "-i", audio,
                    "-filter_complex", full_fc, "-map", "[v]", "-map", f"{len(clips)}:a",
        "-t", str(audio_dur),
                "-threads", "1",
            ...
            if r.returncode == 0:
                ok = True
                break
            print(f"[warn] encode attempt {_attempt+1} failed (rc={r.returncode}), waiting and retrying")
            _t.sleep(20)
        if not ok:
            raise RuntimeError(f"encode failed for {key}")
    import time as _t2
    ok = False
    for _attempt in range(3):
        r = subprocess.run([
        ...
        if r.returncode == 0:
            ok = True
            break
        print(f"[warn] short encode attempt {_attempt+1} failed, retrying"); _t2.sleep(15)
    if not ok:
...
if __name__ == "__main__":
    for idx, (key, scenes) in TRACKS.items():
        dur = duration_of(os.path.join(BASEV2, f"{idx}_{key}.mp3"))
